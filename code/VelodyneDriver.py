...
s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1) # comment this when using window system
s.bind(('', 2368))
# s.connect(('192.168.1.201', 2368)) # uncomment this when using window system
...
channel_data = np.zeros([32, 2, 12])

azimuth_data = np.array([])
distance_data = np.array([])

data_length = 101000

xyz_coordinate = np.zeros((data_length, 3))
coordinate_index = 0
...
while True:
        ...
        for i in range(12):
            ...
            for channel_n in range(32):
                # azimuth_data = np.append(azimuth_data, d_p[i]['channel %d' % (channel_n)][0])  # [azimuth]
                # distance_data = np.append(distance_data, d_p[i]['channel %d' % (channel_n)][1])  # [distance]
                # xyz_coordinate = np.append(xyz_coordinate, get_xyz(channel_n, d_p[i]['channel %d' % (channel_n)][1], d_p[i]['channel %d' % (channel_n)][0]))
                xyz_coordinate[coordinate_index, :] = get_xyz(channel_n, d_p[i]['channel %d' % (channel_n)][1], d_p[i]['channel %d' % (channel_n)][0])
                coordinate_index += 1
                # print(coordinate_index)

        xyz_coordinate = np.reshape(xyz_coordinate, (-1, 3))


        '''
        data preprocessing by filter the zeros data out 
        But the result is only improved little
        '''
        # if coordinate_index > 100000:
        #     mask = np.all(np.equal(xyz_coordinate, 0), axis=1)
        #     xyz_coordinate_mask = xyz_coordinate[~mask]
        #     print("xyz_coordinate_mask.shape: ", xyz_coordinate_mask.shape)
        #     if xyz_coordinate_mask.shape[0] > 100000:
        #         pcd = o3d.geometry.PointCloud()
        #         pcd.points = o3d.utility.Vector3dVector(xyz_coordinate_mask)
        #         vis = o3d.visualization.Visualizer();
        #         vis.create_window(visible=True);
        #         vis.add_geometry(pcd);
        #         # pcd.rotate(pcd.get_rotation_matrix_from_xyz((-0.50, -0.0, 0.1)))
        ...
        if coordinate_index > (data_length - 1000):
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(xyz_coordinate)
            vis = o3d.visualization.Visualizer()
            vis.create_window(visible=True)
            vis.add_geometry(pcd)
            pcd.rotate(pcd.get_rotation_matrix_from_xyz((-1.0, -0.0, 0.1)))
            depth = vis.capture_depth_float_buffer(True)
            # time.sleep(0.01)
            # vis.capture_screen_image('./data collection/result_%f.png' % (time.time()))
            plt.imshow(np.asarray(depth))
            coordinate_index = 0
            # xyz_coordinate = np.zeros((data_length, 3))